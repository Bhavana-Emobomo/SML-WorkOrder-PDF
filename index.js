...
exports.handler = async (event) => {
  try {
    ...
    if (VendorName) {
      ...
      const rowHeight = 0; // Set row height to 0 to remove space between rows
      const cellPadding = 0; // Set cell padding to 0
      let lineHeight = 0; // Set line height to 0 to remove space between lines

      // Draw table headers with no padding
      tableHeaders.forEach((header, index) => {
        currentPage.drawText(header, {
          x: tableXPositions[index], // No padding
          y: itemY, // Direct Y coordinate for header
          size: 9,
          font: timesRomanFontBold,
          color: blackColor,
        });
      ...
      // Draw vertical lines for the table headers (without any space)
      ...
      // Move to next row (right after the header)
      itemY -= rowHeight;
      ...
      listItems.forEach((item) => {
        ...
        if (itemY < footerSpace) {
          ...
          currentPageNumber++;
          itemY = 740; // Reset itemY for new page
        }
        ...
        currentPage.drawText(item.SNO, {
          x: tableXPositions[0], // No padding
          y: rowTextY,
          size: 9,
          font: timesRomanFont,
          color: blackColor,
        ...
        // ItemCode column (draw without vertical space between lines)
        ItemCodeLines.forEach((line, index) => {
          currentPage.drawText(line, {
            x: tableXPositions[2], // Column X-position
            y: rowTextY, // Keep text at the same Y-position for no vertical gap
            size: 9,
            font: timesRomanFont,
            color: blackColor,
          });
        ...
        // WorkOrder column (draw without vertical space between lines)
        WorkOrderLines.forEach((line, index) => {
          currentPage.drawText(line, {
            x: tableXPositions[4], // Column X-position
            y: rowTextY, // Keep text at the same Y-position for no vertical gap
            size: 9,
            font: timesRomanFont,
            color: blackColor,
        ...
        // Description column (draw without vertical space between lines)
        descriptionLines.forEach((line, index) => {
          currentPage.drawText(line, {
            x: tableXPositions[1], // Column X-position
            y: rowTextY, // Keep text at the same Y-position for no vertical gap
            size: 9,
            font: timesRomanFont,
            color: blackColor,
        ...
        // Quantity column (draw without vertical space between lines)
        currentPage.drawText(item.Quantity, {
          x: tableXPositions[3], // Column X-position
          y: rowTextY, // Keep text at the same Y-position for no vertical gap
          size: 9,
          font: timesRomanFont,
          color: blackColor,
        ...
        // Draw right vertical line for the row
        currentPage.drawLine({
          start: {
            x: tableXPositions[tableXPositions.length - 1] + 100,
            y: rowTextY,
        ...
        // Move to next row (adjust Y coordinate)
        itemY -= dynamicRowHeight;
      });

      itemY -= 15; // Reduced spacing at the end (if necessary)
