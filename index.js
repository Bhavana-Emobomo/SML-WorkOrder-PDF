...
exports.handler = async (event) => {
  try {
    ...
    if (VendorName) {
      ...
      listItems.forEach((item) => {
        if (itemY < footerSpace) {
          // Draw continuation message on a new page
          currentPage.drawText(`Continuation of Page ${currentPageNumber}`, {
            x: 400, // Adjust X position for centering
            y: itemY, // Position it at the top of the new page
            size: 12,
        ...
        // Remove dynamic row height logic and keep a fixed row height
        const dynamicRowHeight = rowHeight; // Fixed height for each row

        // Draw cell content (no dynamic line height, fixed height)
        ItemCodeLines.forEach((line, index) => {
          currentPage.drawText(line, {
            x: tableXPositions[2] + cellPadding,
            y: rowTextY - index * lineHeight,
        ...
        itemY -= dynamicRowHeight; // Move Y position down for the next row
      });
